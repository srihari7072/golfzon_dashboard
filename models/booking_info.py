...
class BookingInfo(models.Model):
    ...
    bookg_info_id = fields.Integer("Booking Info ID", required=True, index=True)
    ...
    def get_booking_details(self, limit=80):
        ...
        try:
            bookings = self.search([
                ('deleted_at', '=', False)
            ], limit=limit, order='bookg_info_id desc')
    ...
    def debug_booking_content(self):
        ...
        try:
            ...
            _logger.info(f"🔍 DEBUG: Total active bookings: {self.search_count([('deleted_at', '=', False)])}")
            
            for i, record in enumerate(all_records):
                _logger.info(f"🔍 Record {i+1}: ID={record.bookg_info_id}, Name={ record.bookg_name}, BookingNo={record.bookg_no}, Created={record.created_at}")
    ...
    def get_reservation_chart_data(self, days=30):
        ...
        try:
            ...
            _logger.info(f"📊 Previous Year Period: {prev_year_start} to {prev_year_end}")
            ...
            current_data = self._process_daily_data(current_results, start_date, end_date)
            prev_year_data = self._process_daily_data(prev_year_results, prev_year_start, prev_year_end)
            
            # Calculate statistics
            current_total = sum(current_data)
            prev_year_total = sum(prev_year_data)
            growth_percentage = ((current_total - prev_year_total) / prev_year_total * 100) if prev_year_total > 0 else 0
            ...
            _logger.info(f"📈 Database Results: Current={current_total}, PrevYear={prev_year_total}, Growth={growth_percentage:.1f}%")
    ...
    def get_heatmap_data_with_details(self):
        """Get heatmap data WITH pre-calculated details for instant display"""
        try:
            ...
            if not latest_date_str:
                return self._get_empty_heatmap_with_details()
            
            latest_date = datetime.strptime(str(latest_date_str), '%Y-%m-%d').date()
            start_date = latest_date - timedelta(days=6)  # 7 days total
            
            _logger.info(f"📊 Pre-loading heatmap data: {start_date} to {latest_date}")
            ...
            complete_query = """
                SELECT 
                    ...
                    CASE 
                        WHEN EXTRACT(hour FROM created_at) BETWEEN 5 AND 7 THEN 'Early Morning(5 AM -7 AM)'
                        WHEN EXTRACT(hour FROM created_at) BETWEEN 8 AND 12 THEN 'Morning(8 AM -12 PM)'
                        WHEN EXTRACT(hour FROM created_at) BETWEEN 13 AND 16 THEN 'Afternoon(1 PM -4 PM)'
                        WHEN EXTRACT(hour FROM created_at) BETWEEN 17 AND 19 THEN 'Night(5 PM -7 PM)'
                        ELSE 'Other'
                    END as time_slot,
                    COUNT(*) as booking_count,
                    SUM(COALESCE(play_team_cnt, 1)) as total_teams,
                    COALESCE(play_team_cnt, 1) as individual_team_count
                FROM booking_info 
                WHERE DATE(created_at) BETWEEN %s AND %s
                    AND (deleted_at IS NULL)
                GROUP BY DATE(created_at), EXTRACT(DOW FROM created_at), EXTRACT(hour FROM created_at),
                    CASE 
                        ...
                        ELSE 'Other'
                    END,
                    COALESCE(play_team_cnt, 1)
                ORDER BY booking_date, hour
            """
            
            self.env.cr.execute(complete_query, (start_date, latest_date))
            all_results = self.env.cr.fetchall()
            
            _logger.info(f"📊 Pre-loaded {len(all_results)} database records")
            
            # Process into complete heatmap with details
            heatmap_with_details = self._process_complete_heatmap_data(all_results, start_date, latest_date)
            
            return heatmap_with_details
            
        except Exception as e:
            _logger.error(f"❌ Error pre-loading heatmap data: {str(e)}")
            return self._get_empty_heatmap_with_details()
    ...
    def _process_complete_heatmap_data(self, results, start_date, end_date):
        ...
        for result in results:
            ...
            data_structure[booking_date][time_slot]['hourly_breakdown'][hour] += total_teams
        ...
        for time_index, time_slot in enumerate(time_slots):
            ...
            for day_index in range(7):
                ...
                if current_date in data_structure and time_slot in data_structure[current_date]:
                    team_count = data_structure[current_date][time_slot]['total_teams']
                    hourly_data = data_structure[current_date][time_slot]['hourly_breakdown']
        ...
        _logger.info(f"📊 Pre-calculated {len(cell_details)} cell details for instant access")
